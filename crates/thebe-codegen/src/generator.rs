...
/// 4. Emits `__thebe_render_handler` that calls the handler, renders the
///    template at runtime via `minijinja`, injects the serialised Props JSON,
///    and returns an `axum::response::Html<String>`.
/// 5. Emits `router()` that wires up the Axum route.
pub fn generate_route(
    ...
    source.push_str("\n\n");
    // Store the template as a compile-time constant.
    ...
    source.push_str("    };\n");
    // Inject the Props JSON for the client hydration runtime.
