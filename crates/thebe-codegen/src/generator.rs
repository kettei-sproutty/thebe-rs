...
fn write_module_constants(source: &mut String, literals: ModuleLiterals<'_>) {
    writeln!(source, "const __TEMPLATE: &str = {};", literals.template)
        .expect("infallible");
    writeln!(source, "const __CLIENT_RUNTIME: &str = {};", literals.runtime)
        .expect("infallible");
    write!(
        source,
...
pub fn generate_main(routes: &[RouteEntry]) -> String {
    ...
    for route in routes {
        writeln!(source, "        .merge({}::router())", route.mod_name)
            .expect("infallible");
    }
    source.push_str("    ;\n\n");
    source.push_str(
...
fn parse_handler_signature(
    ...
    let open_paren = after_fn.find('(').ok_or_else(|| {
        CodegenError::InvalidHandlerSignature(method.as_attr_name().to_owned())
    })?;
    let params_start = fn_pos + 3 + open_paren;
    let params_end = find_matching_paren(signature, params_start).ok_or_else(|| {
        CodegenError::InvalidHandlerSignature(method.as_attr_name().to_owned())
    })?;
    ...
    let param_types = split_top_level(params, ',')
        ...
        .map(|param| {
            split_param_type(param).ok_or_else(|| {
                CodegenError::InvalidHandlerSignature(method.as_attr_name().to_owned())
            })
        })
...
mod tests {
    ...
    fn find_handler_returns_error_when_missing() {
        ...
        assert!(matches!(find_handler(setup), Err(CodegenError::MissingHandler)));
    }
    ...
    fn generate_route_uses_declared_http_method_and_forwarded_extractors() {
        ...
        assert!(src.contains("async fn __thebe_render_handler(__thebe_arg0: Path<String>, __thebe_arg1: State<AppState>)"));
    ...
    #[test]
    fn generate_main_uses_path_attributes_for_nested_routes() {
        let source = generate_main(&[
            RouteEntry {
