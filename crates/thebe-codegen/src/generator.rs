...
pub fn generate_main(routes: &[RouteEntry]) -> String {
  ...
  source.push_str("    ;\n\n");
  source.push_str("    let listener = tokio::net::TcpListener::bind(\"0.0.0.0:3000\")\n");
  source.push_str("        .await\n");
  source.push_str("        .expect(\"failed to bind to port 3000\");\n\n");
  source.push_str("    println!(\"thebe dev listening at http://localhost:3000\");\n");
  source.push_str("    axum::serve(listener, app).await.expect(\"server error\");\n");
  source.push_str("}\n");
