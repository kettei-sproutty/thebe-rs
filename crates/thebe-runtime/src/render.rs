...
/// by `MiniJinja`.
pub fn render_template(template: &str, ctx: &serde_json::Value) -> Result<String, RuntimeError> {
  use minijinja::Environment;

  let mut env = Environment::new();
  env
    .add_template("__page", template)
...
mod tests {
  ...
  fn render_simple_binding() {
    let html = render_template("<h1>{{ title }}</h1>", &json!({ "title": "Hello" })).unwrap();
