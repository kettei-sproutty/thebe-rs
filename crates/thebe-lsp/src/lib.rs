...
fn definition_for_document(
  ...
  match find_route_document_match(manifest, relative_path) {
    Some(RouteDocumentMatch::Source(route)) => {
      ...
      if let Some(definition) = generated_client_definition_for_script_position(
        project_root,
        source,
        route,
        position,
...
fn generated_client_definition_for_script_position(
  project_root: &Path,
  source: &str,
  route: &RouteMetadata,
  position: Position,
) -> Result<Option<GotoDefinitionResponse>> {
  let Some(generated_client_path) = route.generated_client_path.as_deref() else {
    return Ok(None);
  };
  let Some(offset) = byte_offset_from_position(source, position) else {
    return Ok(None);
  };
  let Ok(blocks) = parse_source_blocks(source) else {
    return Ok(None);
  };
  ...
  if offset < script_span.start || offset >= script_span.end {
    return Ok(None);
  }

  if let (Some(script_ts), Some(generated_types_path)) = (
    blocks.script_ts.as_deref(),
    route.generated_types_path.as_deref(),
  ) {
    let generated_source = generated_client_mirror_source(script_ts, generated_types_path);
    let generated_offset = generated_client_mirror_prefix_len(generated_types_path)
      + offset.saturating_sub(script_span.start);
    let location = location_for_relative_path_offset(
      project_root,
...
fn generated_client_mirror_source(script_ts: &str, generated_types_path: &str) -> String {
  let props_import_path = generated_client_props_import_path(generated_types_path);
  format!(
...
fn generated_client_mirror_prefix_len(generated_types_path: &str) -> usize {
  generated_client_mirror_source("", generated_types_path).len() - 1
}

fn generated_client_props_import_path(generated_types_path: &str) -> String {
  let prefix = format!("{TYPECHECK_TYPES_DIR}/");
  let relative_types_path = generated_types_path.strip_prefix(&prefix).unwrap_or(generated_types_path);
  let relative_types_path = Path::new(relative_types_path);
  let depth = relative_types_path
    .parent()
    .map_or(0, |parent| parent.components().count());
  let prefix = "../".repeat(depth + 1);
  let target = relative_types_path.with_extension("");
...
fn location_for_relative_path_offset(
  project_root: &Path,
  relative_path: &str,
  source: &str,
  offset: usize,
) -> Result<Location> {
  let position = position_from_byte_offset(source, offset.min(source.len()));
...
"#;
  ...
  #[test]
  fn definition_for_document_returns_generated_client_file_from_client_script_block() {
    let project = TempProject::new();
    let source = format!(
      r#"{}<script lang="ts">
...
"#,
    ...
    let blocks = parse_source_blocks(&source).expect("parse route");
    let script_ts = blocks.script_ts.as_deref().expect("client script");
    let generated_source =
      generated_client_mirror_source(script_ts, ".thebe/types/routes/profile.ts");
    project.write("src/routes/profile.trs", &source);
    project.write(".thebe/client/routes/profile.ts", &generated_source);

    let manifest = fixture_manifest();
    let position = position_from_byte_offset(
      &source,
      source.find("increment").expect("client function") + 2,
    );
    ...
    let expected_position = position_from_byte_offset(
      &generated_source,
      generated_source.find("increment").expect("generated client function") + 2,
    );
