...
- nested generic element nodes plus structured open, close, and self-closing tags for normal HTML tags and PascalCase component tags
- attribute names and quoted/unquoted attribute values, including Thebe surfaces like `:attr`, `on*`, `slot`, and `name`
- HTML comment nodes in template content
...
It is intentionally conservative and does not yet attempt full HTML-aware matching or full embedded Rust/TypeScript/CSS subgrammars. The grammar now exposes nested generic template elements plus injection points for raw block bodies, but the VS Code extension in `packages/thebe-vscode/` still uses the TextMate grammar for editor highlighting today.
